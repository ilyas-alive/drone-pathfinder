...
from classes import Graph, Zone, ZoneType, Connection
...
class Simulation:
    ...
    def run_turn(self) -> list[str]:
        ...
        for d in self.graph.drones:
            if not d.is_delivered():
                target = d.transit_destination.name if d.state == "in_transit" else d.position.name
        ...
        drones = sorted(self.graph.drones, key=lambda x: self.costs.get(x.position.name, float('inf')))
        for d in drones:
            if d.is_delivered() or d.id in moved_this_turn:
                continue

            best_cost = float('inf') if d.position.is_start else self.costs[d.position.name]
            ...
            for n_name, (neighbor, conn) in d.position.neighbors.items():
                if neighbor.zone_type == ZoneType.BLOCKED or n_name not in self.costs:
                    continue
                if not neighbor.is_end and not neighbor.is_start and zone_occupants[n_name] >= neighbor.max_drones:
                    continue
                if connection_usage[conn] >= conn.max_link_capacity:
                    continue

                cost, load = self.costs[n_name], zone_occupants[n_name]
                if cost < best_cost or (cost == best_cost and load < best_load):
    ...
    def run(self) -> list[list[str]]:
        ...
        if not self.graph.start_hub or self.graph.start_hub.name not in self.costs:
        ...
        while not all(d.is_delivered() for d in self.graph.drones):
            ...
            elif any(d.state != "delivered" for d in self.graph.drones) and not any(d.state == "in_transit" for d in self.graph.drones):
